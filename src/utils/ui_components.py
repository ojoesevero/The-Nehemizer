...
import pandas as pd
from src.config.settings import APP_TITLE, APP_SUBTITLE, APP_VERSION, APP_ICON, CUSTOM_CSS
...
def render_header():
    ...
    st.markdown(
        f'<p class="main-title">{APP_ICON} {APP_TITLE} <span class="badge-version">v{APP_VERSION}</span></p>',
...
def render_guia_inicial():
    ...
    st.info("👋 **Bem-vindo ao The Nehemizer!** Para começar, faça o upload do relatório de vendas acima.")
    ...
    with col_g1:
        ...
        * Identificação automática de cabeçalho
        * Reconhece `REFPROD`, `VLRTOTAL`, `RAZAOSOCIAL`
        """)
    with col_g2:
        st.markdown("""
        ### 2️⃣ Contratos BD (PDF)
        * Propostas e contratos Becton Dickinson
        * Extração de preços unitários acordados
        * Suporte a múltiplos PDFs simultâneos
        """)
    with col_g3:
        st.markdown("""
        ### 3️⃣ Tabela Preço Normal
        * Tabela padrão de compras (Excel/CSV)
        * Utilizada para itens sem contrato BD
        * Fallback automático para a aba `NORMAL`
        """)
        
    st.divider()
    
    st.markdown("#### 📥 Precisa de um modelo para testes?")
    template_bytes = gerar_template_exemplo_vendas()
    st.download_button(
        label="📄 Baixar Planilha Modelo de Vendas (.xlsx)",
        data=template_bytes,
        file_name="Template_Vendas_TheNehemizer_Exemplo.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        help="Baixe esta planilha de exemplo com colunas prontas para testar o sistema."
    )
...
def render_kpis_e_graficos(df: pd.DataFrame):
    ...
    with kpi3:
        ...
        st.metric("⚠️ Itens na Aba NORMAL", f"{total_normal:,}", delta=f"-{pct_norm:.1f}%", delta_color="inverse")
    with kpi4:
        st.metric("💰 Total Vendas (R$)", f"R$ {vlr_total_venda:,.2f}")

    # Gráficos de Apoio
    with st.expander("📊 Visualizar Distribuição de Vendas e Contratos", expanded=False):
        ...
        with col_c1:
            ...
            st.bar_chart(chart_data, color="#F37021")
        ...
        with col_c2:
            ...
            st.bar_chart(status_data, color="#333333")
